          ...
          <div key={archiveTitle}>
            ...
            <ul>
              {archivePosts[archiveTitle].map(post => (
                ...
                >
                  <div id={post?.date?.start_date} className="text-lg font-bold text-center">
                    {`${new Date(post?.date?.start_date).getFullYear()} · ${String(new Date(post?.date?.start_date).getMonth() + 1).padStart(2, '0')}`}
